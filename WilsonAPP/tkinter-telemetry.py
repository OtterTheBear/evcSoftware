...
class App(tk.Frame):
    def __init__(self, master):
        ...
        self.nowLapTime = tk.StringVar()
        self.lapButtonText = tk.StringVar(value="Start Lap")
        ...
        #add gui for lap tracking
        tk.Label(self.frameNow, text="Lap Time").grid(column=0, row=17, padx=10, pady=(10, 0))
        tk.Entry(self.frameNow, width=13, justify="right", textvariable=self.nowLapTime).grid(column=1, row=17, padx=10, pady=(10, 0))

        self.lapButton = tk.Button(self.frameNow, textvariable=self.lapButtonText, command=self.record_lap)
        self.lapButton.grid(column=0, row=18, columnspan=2, padx=10, pady=(10, 20))
        ...
        # Add Lap Variables for tracking
    ...
    def record_lap(self):
        ...
        if self.lap_start_time is None:
            ...
            self.lapButtonText.set("End Lap")
            self.nowLapTime.set("00:00")
        else:
            # End current lap
            lap_duration = now - self.lap_start_time
            self.lap_start_time = None
            self.lapButtonText.set("Start Lap")
            minutes = int(lap_duration // 60)
            seconds = int(lap_duration % 60)
            lap_str = f"{minutes:02d}:{seconds:02d}"
            self.nowLapTime.set(lap_str)
            self.lap_times.append(lap_duration)
            print(f"Lap {len(self.lap_times)}: {lap_str}")
...
if playbackfilename is not None:
    ...
    ser = serial.Serial(serialport, 9600, timeout=0)  # non-blocking

    def poll_serial():
        try:
            data = ser.read(256)  # read up to 256 bytes, non-blocking
            if playbackfilename is None and data:
                logfile.write(data)
            if data:
                process_rawdata(data)
        ...
        # schedule next poll (every 10 ms for example)
...
np.savez( npz_filename,
	...
	data_volts        = myapp.data_volts,
        ...
        lap_times         = np.array(myapp.lap_times))
if playbackfilename is None:
    logfile.close()
    if myapp.lap_times:
        with open("lap_times.txt", "w") as f:
            for i, lap in enumerate(myapp.lap_times, 1):
                f.write(f"Lap {i}: {lap:.2f} sec\n")
